    ...
    <div class="mx-auto max-w-7xl">
      <header class="grid gap-8 border-b border-[var(--theme-border)] pb-8 lg:grid-cols-[1.3fr_0.7fr]">
        ...
        <div class="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
          <div class="border border-[var(--theme-border)] bg-[var(--theme-surface)] p-4">
            <BriefcaseIcon class="mb-3 h-5 w-5 text-[var(--theme-accent-soft)]" />
            <p class="text-2xl font-semibold">{{ experienceData.length }}</p>
            <p class="text-sm text-[var(--theme-text-dim)]">Experience entries</p>
          </div>
          <div class="border border-[var(--theme-border)] bg-[var(--theme-surface)] p-4">
            <Code2Icon class="mb-3 h-5 w-5 text-[var(--theme-success)]" />
            <p class="text-2xl font-semibold">{{ totalTechnologies }}</p>
            <p class="text-sm text-[var(--theme-text-dim)]">Technologies listed</p>
          </div>
          <a
            :href="resumeUrl || '#'"
            target="_blank"
            ...
            class="flex items-center justify-between border border-[var(--theme-accent)] bg-[rgb(var(--theme-accent-rgb)/0.12)] p-4 text-[var(--theme-accent-soft)] transition hover:bg-[rgb(var(--theme-accent-rgb)/0.18)]"
          >
        ...
        <main class="space-y-12">
          <section>
            <div class="mb-6 flex items-center gap-3">
              <BriefcaseIcon class="h-6 w-6 text-[var(--theme-accent-soft)]" />
            ...
            <div class="space-y-6">
              <article
                ...
                class="border-l-2 border-[var(--theme-accent)] pl-5"
              >
                <div class="flex flex-wrap items-baseline justify-between gap-3">
                  <div>
                    ...
                    <p class="mt-1 text-[var(--theme-text-dim)]">
                      {{ experience.company }} - {{ experience.position }}
                    </p>
                  </div>
                  <p class="text-sm text-[var(--theme-text-disabled)]">
                    {{ experience.dateStart }} - {{ experience.dateEnd }}
                  </p>
                </div>
                <ul class="mt-4 list-disc space-y-3 pl-5 text-[var(--theme-text-soft)]">
          ...
          <section>
            <div class="mb-6 flex items-center gap-3">
              <GraduationCapIcon class="h-6 w-6 text-[var(--theme-accent-soft)]" />
            ...
            <article
              ...
              class="border border-[var(--theme-border)] bg-[var(--theme-surface)] p-5"
            >
              <h3 class="text-2xl font-semibold text-orange-100">{{ education.degree }}</h3>
              <p class="mt-2 text-[var(--theme-text-soft)]">{{ education.program }}</p>
              <p class="mt-2 text-[var(--theme-text-dim)]">
        ...
        <aside>
          <div class="sticky top-6 space-y-5">
            <section
              ...
              class="border border-[var(--theme-border)] bg-[var(--theme-surface)] p-5"
            >
              <h2 class="text-xl font-semibold text-[var(--theme-success)]">{{ stack.category }}</h2>
              <p class="mt-2 text-sm leading-6 text-[var(--theme-text-dim)]">{{ stack.description }}</p>
              <div class="mt-4 flex flex-wrap gap-2">
                <span
                  v-for="tech in stack.technologies"
                  :key="tech.name"
                  class="border border-[var(--theme-border-strong)] px-2 py-1 text-sm text-[var(--theme-text)]"
                >
                  {{ tech.name }}
                </span>
              </div>
